...
&emsp;&emsp;从更“工程”的角度看，一整个 CNN 可以大致分成“**特征提取**”和“**分类器**”两个部分：前面若干个卷积 + 池化层负责在空间上提取越来越抽象的特征图；最后通过扁平化（Flatten）接到一两层全连接层上完成分类。下面这张 cartoon 图刚好展示了这个流程，你可以放在本小节的末尾，帮助读者建立对 CNN 的整体直觉。
