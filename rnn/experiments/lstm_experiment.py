# rnn/experiments/lstm_experiment.py
"""
LSTM 分类器 CIFAR-10 实验
...
def plot_training_curves(history: dict, save_path: Path):
    ...
    ax1.plot(loss_epochs, losses, linewidth=2, color='#e74c3c')
    ax1.set_title("LSTM Training Loss", fontsize=14, fontweight='bold')
    ax1.set_xlabel("Epoch")
    ax1.set_ylabel("Loss")
    ax1.grid(True, alpha=0.3)

    ax2.plot(epochs, train_acc, marker="o", label="Train Acc", linewidth=2, color='#3498db')
    ax2.plot(epochs, val_acc, marker="s", label="Val Acc", linewidth=2, color='#2ecc71')
    ax2.set_title("LSTM Accuracy", fontsize=14, fontweight='bold')
...
def main():
    print("=" * 60)
    print("🚀 CIFAR-10 LSTM Experiment")
    print("=" * 60)
    
    results_dir = current_dir / "lstm_results"
    ...
    # Save model
    model_path = results_dir / "lstm_model.npy"
    model.save_model(str(model_path))
    print(f"✓ Saved model: {model_path}")

    # Save history
    hist_path = results_dir / "lstm_history.npy"
    ...
    pred_path = results_dir / "lstm_prediction_viz.png"
    class_names = get_cifar10_class_names()
    visualize_predictions(X_test, y_test, model, class_names, pred_path,
                          num_samples=CONFIG["visualization"]["num_pred_samples"])

    # Save report
    report_path = results_dir / "lstm_experiment_results.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("CIFAR-10 LSTM Experiment\n")
        f.write("=" * 60 + "\n")
